...
def get_peaks(rates, cmpf):
    ...
    for gr in range(len(rates)):
        ...
        # peaks.append(peakdet(r,thresh)[0]) # FIX where's peakdet??
    return peaks


def get_singleprop(peaks, max_delay, min_delay, t_sec=50, evoked_p=None,
                   detect_bursts=True):
    ...
    for i, c in enumerate(peaks):
        ...
        if not fl:
            break  # finish with this stimulation
    return cnt
...
def get_prev(gr_peaks, t, max_delay, min_delay):
    ...
    v_t = v[0]
    for v in gr_peaks:
        if (t - v[0]) > max_delay or (t - v[0]) < min_delay:
            continue
        flg = True
        v_t = v[0]
        if flg:
