...
def get_pairwise_corr(nn, gr_spike_m):
    mean_corr = 0.
    for m1 in range(nn.s_ass):
        print(m1)
        s1 = get_spike_train(gr_spike_m[m1])
        for m2 in range(m1):
            s2 = get_spike_train(gr_spike_m[m2])
...
def get_activ(nn, halfp=200, tstim=19.):
    ch = 0
    dt = nn.m_ts / ms
    ...
    for g in range(nn.n_ass):
        ...
        for j in range(nn.s_ass):
            ...
            stimes.extend([x for x in times if t0_dis < x < t1_dis])
        print(peak_x, peak_y)
...
def get_actpeaks(nn):
    ...
    for gr in range(nn.n_ass):
        ...
        # act_peaks.append(peakdet(r, thresh)[0]) # FIX where's peakdetect?
    return act_peaks


def get_singleprop(peaks, t_sec=50, evoked_p=None):
...
def get_spontprop(peaks, evoked, first=50, last=100):
    ...
    flg = None
    for v in peaks[-1]:
        if v[0] < first * 1000 / t_res:
            continue
        if v in evoked:
            continue
        if v[0] > last * 1000 / t_res:
            break
        t = v[0]
